...
const stateMediator = {
  ...
  mediaMuted: {
    ...
    set(value, stateOwners) {
      ...
      media.muted = value;
      try {
        const hasLocalStoragePrefMuted = import_server_safe_globals.globalThis.localStorage.getItem("media-chrome-pref-muted") !== null;
        const hasMutedAttribute = media.hasAttribute("muted");
        if (noMutedPref) {
          if (hasLocalStoragePrefMuted)
            import_server_safe_globals.globalThis.localStorage.removeItem("media-chrome-pref-muted");
          return;
        }
